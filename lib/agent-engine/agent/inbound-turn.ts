...
import { fecharCheckpoint } from './fechar-checkpoint';
...
export async function runAgentTurn(
  ...
  // fechar-checkpoint.ts. Antes, JSON inválido lançava e a fila refazia o turno
  // INTEIRO com a resposta já enviada, travando a próxima mensagem do contato.
  const fechamento = await fecharCheckpoint({
    chamar: async () => {
      ...
      return { text: closing.result.text, callId: closing.callId };
    },
    interpretar: parseCheckpointText,
    aoFalhar: (tentativa, err) => {
      // Só o motivo (o trecho antes do " — "), nunca o texto do modelo: pode
      // carregar PII da conversa. O sufixo "run re-tentado pela fila" das
      // mensagens do parser não vale mais aqui e confundiria quem lê o log.
      runLog.warn('checkpoint: tentativa de fechamento falhou', {
        tentativa,
        motivo: (err.message.split(' — ')[0] ?? '').slice(0, 120),
      });
    },
  });

  if (fechamento === null) {
    runLog.warn('checkpoint descartado — turno concluído com a memória do checkpoint anterior');
  } else {
    const { content, callId } = fechamento;
    ...
    if (mudanca.emit) {
      try {
        const r = await emitAgentActivityForContact({
          ...
          ...(callId ? { evidence: { llm_call_ids: [callId] } } : {}),
